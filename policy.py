...
def set_rules(policies, overwrite=True):
    ...
    for pol in policies.keys():
        if any([pol.startswith(depr_pol) for depr_pol in
            ...
            LOG.warn(("Found deprecated policy rule:%s. Please consider "
            ...
            try:
                ...
                for actual_policy in ['_'.join(item) for item in
                    ...
                    if actual_policy not in policies:
                        ...
                        LOG.info(("Inserting policy:%(new_policy)s in "
                                     "place of deprecated "
                                     "policy:%(old_policy)s"),
                ...
                LOG.error(("Backward compatibility unavailable for "
...
def _build_subattr_match_rule(attr_name, attr, action, target):
    ...
    if not key:
        LOG.warn(("Unable to find data type descriptor for attribute %s"),
                 attr_name)
        return
    data = validate[key[0]]
        ...
        if self.target_field not in target:
            ...
            try:
                ...
                with excutils.save_and_reraise_exception():
                    LOG.exception(('Policy check error while calling %s!'),
                                  f)
